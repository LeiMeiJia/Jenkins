package com.aerozhonghuan.java.thread.callable;
...
public class MyFutureTask extends FutureTask<Integer> {
    ...
    public MyFutureTask(@NonNull Callable<Integer> callable) {
        super(callable);
    }
    ...
    protected void done() {
        ...
        try {
            int result = get();
            Log.d(TAG, Thread.currentThread().getName() + "--->" + sdf.format(new Date()) + "--->获取任务结果：" + result);
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
        } catch (CancellationException e) {
            Log.d(TAG, Thread.currentThread().getName() + "--->任务被取消于：" + sdf.format(new Date()));
        }
    }
}
