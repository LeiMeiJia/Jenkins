...
import com.aerozhonghuan.demo.mvp.model.retrofit.RetrofitApi;
...
//        Handler handler = new Handler(Looper.getMainLooper());
    ...
    // 发送网络请求，请求执行在子线程
    public void sendHttpRequest(Call<ResponseBody> call, NetCallback callback) {
        // 每个Call实例可以且只能执行一次请求，不能使用相同的对象再次执行execute()或enqueue()。
        CallAdapter callAdapter = new CallAdapter(callback);
